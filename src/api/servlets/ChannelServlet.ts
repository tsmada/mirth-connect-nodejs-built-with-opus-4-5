...
channelRouter.get(
  '/',
  authorize({ operation: CHANNEL_GET_CHANNELS }),
    ...
    try {
      ...
      if (includeCodeTemplateLibraries) {
        for (const channel of channels) {
          channel.codeTemplateLibraries = await ChannelController.getCodeTemplateLibraries(
            channel.id
          );
        }
      }
...
channelRouter.get(
  '/idsAndNames',
  authorize({ operation: CHANNEL_GET_IDS_AND_NAMES }),
...
channelRouter.get(
  '/:channelId',
  authorize({ operation: CHANNEL_GET_CHANNEL, checkAuthorizedChannelId: 'channelId' }),
  async (req: Request, res: Response) => {
    try {
...
channelRouter.post(
  '/',
  authorize({ operation: CHANNEL_CREATE }),
  async (req: Request, res: Response) => {
    try {
...
channelRouter.put(
  '/:channelId',
  authorize({ operation: CHANNEL_UPDATE, checkAuthorizedChannelId: 'channelId' }),
  async (req: Request, res: Response) => {
    try {
      const channelId = req.params.channelId as string;
      const channelData = req.body;
...
channelRouter.delete(
  '/_removeAllMessages',
  authorize({ operation: MESSAGE_REMOVE_ALL }),
  async (req: Request, res: Response) => {
    try {
      const channelIds = req.query.channelId;
      ...
      for (const channelId of ids) {
        ...
        if (clearStatistics) {
          try {
            await pool.execute(
              `UPDATE D_MS${tableId} SET RECEIVED = 0, FILTERED = 0, TRANSFORMED = 0, PENDING = 0, SENT = 0, ERROR = 0`
            );
          } catch {
...
channelRouter.delete(
  '/:channelId',
  authorize({ operation: CHANNEL_REMOVE, checkAuthorizedChannelId: 'channelId' }),
  async (req: Request, res: Response) => {
    try {
      const channelId = req.params.channelId as string;
...
channelRouter.delete(
  '/',
  authorize({ operation: CHANNEL_REMOVE }),
  async (req: Request, res: Response) => {
    try {
      const channelIds = req.query.channelId;
      if (!channelIds) {
...
channelRouter.post(
  '/_setInitialState',
  authorize({ operation: CHANNEL_UPDATE }),
  async (req: Request, res: Response) => {
    try {
      const { channelIds, initialState } = req.body as {
        channelIds?: string[];
        initialState?: string;
      };
 ...
 */
channelRouter.post(
  '/:channelId/initialState/:initialState',
  authorize({ operation: CHANNEL_UPDATE, checkAuthorizedChannelId: 'channelId' }),
  async (req: Request, res: Response) => {
    try {
      const channelId = req.params.channelId as string;
